...
from models.modules.common import MSFE,MSFE_v2
import yaml
with open('config.yaml', 'r') as f:
    config = yaml.load(f, Loader=yaml.FullLoader)

MSFE_en =  config['MSFE_en']
class DenseBlock(nn.Module):
    def __init__(self, channel_in, channel_out, init='xavier', gc=32, bias=True):
        super(DenseBlock, self).__init__()
        self.MSFE_en = MSFE_en
        if self.MSFE_en:
            self.MSFE = MSFE(gc,gc)
...
def subnet(net_structure, init='xavier'):
    def constructor(channel_in, channel_out, groups=None):
        ...
        elif net_structure == 'MSFE':
            if init == 'xavier':
                return MSFE(channel_in, channel_out)
            elif init == 'xavier_v2':
                return MSFE_v2(channel_in,channel_out,groups)
            else:
                return MSFE(channel_in,channel_out)
        else:
            return None
